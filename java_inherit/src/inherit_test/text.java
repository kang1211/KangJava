...
//	   - 여러개의 클래 중에서 부모 클래스는 하나이다. 
//	   - 클래스에서 상속은 단일상속만 가능(부모 클래스는 하나 뿐)
// 	   - 부모클래스의 인스턴스변수, 메서드를 자식 클래스에서 사용 할 수 있다.
//	   - 단, final이 붙은 메서드는 오버라이딩이 안됨
...
// 다형성 - 부모클래스로 자식클래스의 객체를 담을 수 있는 방법
//		- 객체의 메서드가 통일되어 사용될 수 있다.
//		- 하나의 타입으로 여러 타입을 관리 할 수 있기 때문에 유지보수가 좋다.
//		- 객체의 재사용성이 쉬워지기 때문에 코드의 재상용성이 높아져 개발자가 편하다.
//		- 안정성이 높다.
...
//			   부모 클래스의 메서드를 자식클래스에서도 구현 되어야 한다.
//			   자식클래스에 오버라이딩 된 메서드 위에는 @Override라고 명시해주는게 좋다.
//			   부모클래스로부터 상속받아 재구성된 메서드라고 표기해줘야 다른개발자도 알아보기 편하고
//			   컴퓨터가 실행하는데 올바른 방식으로 실행시킬 수 있다.
//			 : @Override를 작성하면 컴퓨터는 오버라이딩을 직접 검수할 필요 없이 바로 상속
//			   을 인지하고 오버라이딩을 실행한다. 작성안하면 직접 검수후 실행하기에 컴퓨터의 작업량이 증가함
// - 업캐스팅 : 상위 타입으로 타입 변환
...
//		자식클래스가 추상메서드를 실체화 구현 하지않으면 지식클래스도 추상클래스가 되어야한다.
//		추상 클래스는 객체 생성이 안된다.