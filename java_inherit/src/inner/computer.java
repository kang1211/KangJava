...
//			computer.this.print();
	...
	public void OS_install() {
		...
		class OS{ // 로컬 클래스
			public void getOs() {
//				version=10; // 로컬 클래스에서 지역변수는 final이 붙은 상수로 사용된다. 그래서 값 변경 불가하다.
...
//				 모두 선언이 가능하다.
//			   - 외부클래스의 인스턴스 변수, 메서드의 사용은 불가능하다.
//			   - 외부 클래스의 객체를통한 객체 생성이 아니라 독자적으로 생성 가능
//			   - static변수, 메서드는 메모리에 하나만 올라가지만 
//			     static 클래스는 여러개 생성이 가능하다.
//			   - 외부 클래스의 인스턴스를 사용하지 않는 내부클래스라면
//				 내부클래스를 static으로 선언한다. (일부IDE개발도구에서는 경고뜬다)
