...
public class java_test {
	public static void main(String[] args) {
		...
		for(int i=0;i<list.size();i++) {// 배열의 크기는 size 메서드로 알 수 있다.
			System.out.println(list.get(i));//ArrayList에 저장되어있는 배열의 값을 가져오려면
		}
		// 도서 검색 - 검색어와 일치하는 값 찾기, 검색어가 포함되어있는 값 찾기
		// 책 제목으로 검색 - 검색어와 일치하는 값 찾기
		// 검색 -> 비교(일치하는 값 찾기 - 같다, 같지않다	포함 값 찾기 - 범위 검색(같다, 같지않다)
		// 일치하는 값 -> indexOf, 포함 -> contains
		
		book tmp = new book("자전거라이딩","김정환",2024);
		
		System.out.println(list.indexOf("자전거라이딩"));
		
		//정렬 - 두개의 데이터를 비교하여 크고작다를 따져서 위치를 변경하는 방법
