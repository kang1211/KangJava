...
public class linkedList {
	...
	static int cnt = 0;
	private node head=null;//링크드 리스트의 
	private node tail=null;//링크드 리스트의
...
//-----------------------------------------------------------------------	
	...
	public node get(int index) {
		...
		while(find!=null){
			...
			find = find.getNext();//현재노드의 다음노드로 이동
		}
		return null;
	}
//-----------------------------------------------------------------------
	...
	public boolean remove(int index) {
		...
		if(find.getNext().index == index) {
			...
			find.setNext(del.getNext());// del의 next를 앞쪽 node에 저장
			reindex();
			cnt--;
			return true;
		}
...
//-----------------------------------------------------------------------	
	...
	public void add(int index, String name) {
		...
		if(index==0) {//링크드리스트 맨앞에 추가삽입한다면
			...
			//다음주석 설명은 만약 4번째 위치에 새노드 삽입한다면에 대한 설명
			while(find.getNext()!=null) {
				if(find.getNext().index==index) {//find는 3번째 노드이다.
					temp.setNext(find.getNext());
					//새로운 노드의 next에 3번째 노드의next(4번째)를 저장한다.
					find.setNext(temp);
